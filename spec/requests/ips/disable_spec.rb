...
RSpec.describe 'POST /ips/:id/disable', type: %i[request database] do
  ...
  let!(:ip_id) { ips.insert(enabled: true) }
  ...
  context 'for existing ip' do
    before { post "/ips/#{ip_id}/disable", {}.to_json, request_headers }
    ...
    it 'returns valid response', :aggregate_failures do
      ...
      expect(JSON.parse(last_response.body)['message']).to eq "IP #{ip_id} is disabled"
    end
  end
end
